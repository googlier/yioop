    ...
    public function start()
    {
        ...
        if (isset($argv[1]) && $argv[1] == "start") {
            if (isset($argv[2]) && in_array($argv[2],
                ...
                CrawlDaemon::init($argv, $this->process_name, 3);
            } else {
                $argv[2] = "none";
                $argv[3] = self::INDEXER;
                CrawlDaemon::init($argv, $this->process_name, 0);
                $argv[2] = "none";
                $argv[3] = self::SCHEDULER;
                CrawlDaemon::init($argv, $this->process_name, 2);
            }
        } else {
            CrawlDaemon::init($argv, $this->process_name);
        }
